...
alter table public.card_templates
  ...
  check (rarity in ('COMMON', 'RARE', 'EPIC', 'LEGENDARY', 'MYTHIC', 'ULTRA_LEGENDARY', 'DIVINE'));
...
alter table public.currency_transactions
  ...
  check (reason in ('DAILY_LOGIN', 'MATCH_WIN', 'MATCH_LOSS', 'MATCH_DRAW', 'PACK_PURCHASE', 'GACHA_PULL', 'ADMIN_GRANT'));
...
values (
  'necrp-tuff-edition',
  'necrp (tuff edition)',
  'The featured constellation leader reward.',
  'DIVINE',
  'LEADER',
  12,
  15,
  4,
  12,
  '/card-art/necrp-tuff-edition.webp',
  'Tuff enough to bend the pity counter.',
  '[]'::jsonb,
  'prototype-0.1'
)
...
create table if not exists public.gacha_pity (
  ...
  pulls_since_featured integer not null default 0 check (pulls_since_featured >= 0),
  total_pulls integer not null default 0 check (total_pulls >= 0),
  featured_copies integer not null default 0 check (featured_copies >= 0),
  updated_at timestamptz not null default now(),
  primary key (user_id, banner_slug)
);
...
create table if not exists public.gacha_pull_history (
  ...
  featured_hits integer not null default 0,
  created_at timestamptz not null default now()
);

create index if not exists gacha_pity_user_banner_idx
on public.gacha_pity(user_id, banner_slug);

create index if not exists gacha_pull_history_user_created_idx
on public.gacha_pull_history(user_id, created_at desc);

alter table public.gacha_pity enable row level security;
alter table public.gacha_pull_history enable row level security;

drop policy if exists "players read own gacha pity" on public.gacha_pity;
create policy "players read own gacha pity"
on public.gacha_pity for select
to authenticated
using (user_id = auth.uid());

drop policy if exists "players read own gacha history" on public.gacha_pull_history;
create policy "players read own gacha history"
on public.gacha_pull_history for select
to authenticated
using (user_id = auth.uid());
...
create or replace function public.grant_gacha_pulls(
  ...
  p_hard_pity integer default 50
)
returns table (
  coins integer,
...
begin
  ...
  for pull_index in 1..p_pull_count loop
    ...
    if next_pity >= p_hard_pity then
      featured_chance := 1;
    elsif next_pity <= 25 then
      featured_chance := 0.005;
    else
      featured_chance := least(0.65, 0.005 * power(1.35, next_pity - 25));
    end if;
