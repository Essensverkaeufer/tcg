...
export function DeckBuilderClient() {
  ...
  const traitOptions = useMemo(() => {
    ...
    for (const owned of ownedCards) getVisibleTraits(owned.card).forEach((value) => values.add(value));
  ...
  const visibleOwnedCards = useMemo(() => {
    const matchingCards = ownedCards
      .filter((owned) => {
        ...
        const matchesTrait = trait === "ALL" || getVisibleTraits(owned.card).includes(trait);
  ...
  }, [cardType, deck, ownedCards, rarity, search, sortDirection, sortKey, trait]);
    ...
    <AuthGate>
      <main className="page-enter mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_340px]">
        <section>
          ...
          <div className="mt-6 grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-6">
