...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="app-header-enter border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <Link href="/" className="brand-pulse flex items-center gap-2 text-lg font-black">
          ...
          <nav className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-600">
            {navItems.map(([label, href]) => (
              <Link key={href} href={href} className={clsx("animated-nav-link rounded-md px-3 py-2 hover:bg-slate-100 hover:text-slate-950", pathname === href && "animated-nav-link-active")}>
                {label}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-3 text-sm">
            ...
            <label
              ...
              className="music-control inline-flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-xs font-black text-slate-700"
            >
              ...
              <input
                ...
                className="music-slider h-2 w-24 cursor-pointer accent-rose-600 disabled:cursor-not-allowed disabled:opacity-45"
              />
            </label>
            <button
              type="button"
              ...
              className="skip-button rounded-md border border-slate-200 p-2 text-slate-700 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-45"
            >
              <SkipForward className="h-4 w-4" aria-hidden />
            </button>
            <span className="coin-counter inline-flex items-center gap-1 rounded-md border border-amber-200 bg-amber-50 px-2 py-1 font-bold text-amber-800">
